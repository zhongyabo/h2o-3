...
# From PUBDEV-5708 GBM not stopping early if logloss is specified

def gbm_residual_deviance():

  airlineData = h2o.import_file(path=pyunit_utils.locate("smalldata/airlines/AirlinesTest.csv.zip"))
  gbm = H2OGradientBoostingEstimator(ntrees=10000, stopping_rounds=5, stopping_metric='logloss', stopping_tolerance=0.001)
  resp = 'IsDepDelayed'
  gbm.train(x=list(range(9)), y=resp, training_frame=airlineData, validation_frame=airlineData)
