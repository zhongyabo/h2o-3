...
public abstract class SharedTreeModel<
  ...
  @Override public ModelMetricsSupervised.MetricBuilderSupervised makeMetricBuilder(String[] domain) {
    return makeMetricBuilder(domain, false);
  }

  @Override public ModelMetricsSupervised.MetricBuilderSupervised makeMetricBuilder(String[] domain, boolean finalScore) {
    switch(_output.getModelCategory()) {
      case Binomial: return new ModelMetricsBinomial.MetricBuilderBinomial(domain, setAUCWorkingBinSize(_parms.train()==null?1:_parms.train().anyVec().nChunks(), finalScore));
