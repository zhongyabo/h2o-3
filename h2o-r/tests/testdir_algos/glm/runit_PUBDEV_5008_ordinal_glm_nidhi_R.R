...
glmOrdinal <- function() {
  ...
  objreg = 1/h2o.nrow(D)
  objregs = c(objreg, objreg/10, objreg/100, objreg/1000, objreg/10000)#, objreg/100, objreg/1000, objreg/10000, objreg/100000)
  lambdaR = c(0)
  betaL = c(0,0.5,1)
  for (lambdaR in objreg) {
    for (beta in betaL) {
  for (objR in objregs) {
    m1 <- h2o.glm(y = Y, x = X, training_frame = D, lambda=c(lambdaR), family = "ordinal", beta_epsilon=1e-8, 
                objective_epsilon=1e-10, obj_reg=objR, alpha=c(beta))  
