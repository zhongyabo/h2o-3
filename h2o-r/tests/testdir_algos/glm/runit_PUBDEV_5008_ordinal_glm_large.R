...
glmMultinomial <- function() {
  ...
  m1 <- h2o.glm(y = Y, x = X, training_frame = D, family = "ordinal", alpha = 0.99, solver='IRLSM')  
  m2 <- h2o.glm(y = Y, x = X, training_frame = D, family = "ordinal", alpha = 0.99, solver = 'L_BFGS',max_iter=500)  
  print(m1)
  print(m2)  
